...
class HomeController extends Controller
{
    ...
    public function setoran(Request $request){
        $setor = M_transaksi::select(DB::raw("DATE_FORMAT(transaksi.waktu,'%e') as tgl"),DB::raw("SUM(transaksi.nominal) as nom"))
                ->where('transaksi.jnstransaksi','=','setor')
                ->where(DB::raw("DATE_FORMAT(transaksi.waktu,'%Y-%m')"),'=',date('Y-m'))
                ->groupBy(DB::raw("DATE_FORMAT(transaksi.waktu,'%Y-%m-%d')"))->get();
