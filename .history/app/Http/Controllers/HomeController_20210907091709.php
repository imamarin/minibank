...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        $data = array(
            ...
            'setor' => M_transaksi::select(DB::raw(SUM('transaksi.nominal')))->where(DB::raw("DATE_FORMAT(transaksi.waktu,'%Y-%m')"),'=',date('Y-m'))->where('transaksi.jnstransaksi','=','setor'),
            'tarik' => M_transaksi::select(DB::raw(SUM('transaksi.nominal')))->where(DB::raw("DATE_FORMAT(transaksi.waktu,'%Y-%m')"),'=',date('Y-m'))->where('transaksi.jnstransaksi','=','tarik')
