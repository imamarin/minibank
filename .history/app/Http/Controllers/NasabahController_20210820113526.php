    ...
    public function ubahpassword(Request $request){
        ...
        if($queri>0){
            if($request->passbaru == $request->confirmpass){
                $updateRow=DB::table('users')->where('username','=',$request->username)->update([
                    'password' => sha1($request->passbaru),
                ]);
            }           
            
            return redirect('/rekening')->with(['success' => 'Berhasil diubah!']);
        }else{
            return redirect('/rekening')->with(['warning' => 'Tidak berhasil diubah!']);
        }
    }
}
