...
class NasabahController extends Controller
{
    ...
    public function pembayaran(){
        if(session()->get('kategori')=='siswa'){
            ...
            // $queri = M_subrekening::where('norek','=','7093332223')->where('thnpembayaran','=',$siswa->thnmasuk)->where('kategori','=',session()->get('kategori'))->get();
            $queri = DB::table('grouppembayaran as gp')->select('sr.*')->join('subrekening as sr','sr.idsubrekening','=','gp.idsubrekening')
                    ->where('gp.norek','=',session()->get('norek'))->get();
            
        }else{
            $queri = M_subrekening::where('norek','=','7093332223')->get();
        }
        $data = array(
            'pembayaran' => $queri,
            'pembayaran2' => M_subrekening::where('norek','=','7093332223')->where('kategori','=','umum')->get()
        );
    ...
    public function update(Request $request){
        ...
        if($queri->count()>0){          
            ...
            if($request->kategori=='siswa'){
                DB::table('siswa')->where('nin',$request->nin)->update([
                    'nisn' => $request->nisn,
                    'thnmasuk' => $request->thnmasuk
                ]);
            }
