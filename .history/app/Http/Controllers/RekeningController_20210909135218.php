...
class RekeningController extends Controller
{
    ...
    public function koran(){
        $data = array(
            'transaksi' => M_transaksi::select('transaksi.*','nasabah.*','subrekening.subrekening','transaksi.iduser as paraf'
            DB::raw("(SELECT nsb.nama FROM nasabah as nsb, rekening as rk WHERE nsb.nin = rk.nin AND rk.norek=transaksi.norektujuan) as penerima"))
                    ...
                    ->where('rekening.status','=','1')
                    ->where(DB::raw("DATE_FORMAT(transaksi.waktu,'%Y-%m-%d')"),'=',date('Y-m-d'))
    ...
    public function cetak(Request $request){
        $data=array(
            'rekening' => M_rekening::join('nasabah','nasabah.nin','=','rekening.nin')->where('rekening.norek','=',$request->norek)->get()
        );
        return view('admin/cetakidentitas')->with($data);
    }
