...
class ReqinvoiceController extends Controller
{
	...
	public function test_curl(){
		$query = DB::table('nasabah')->join('rekening','rekening.nin','=','nasabah.nin')->offset(555)->limit(0)->get();
		$data=array();
		foreach ($query as $key => $value) {
			# code...
			$urut = sprintf("%05d", $key+1);
			$data[$key] = array(
				"number" =>"TBN/2021/09/".$urut,
				"va"=> $value->norek,
				"date"=> date('Y-m-d'),
				"amount"=> 0,
				"name"=> $value->nama,
		...
		// // $str = file_get_contents('https://minibank.smk-ypc.sch.id/nasabah.json');
		// // var_dump($str);
