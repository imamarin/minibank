...
<table border="0" cellspacing="0">
    <tbody>
        ...
        @foreach($transaksi as $data)
            ...
            @if($no >= $rec1)
            <tr style="height:30px;font-size: 12px;">
                <td style="width: 75px;text-align:right;">{{ date('d/m/Y', strtotime($data->waktu)) }}</td>
                <td style="width: 56px;text-align:center;">{{ $data->sandi }}</td>
                <td style="width: 94px;text-align:right;">{{ $debit }}</td>
                <td style="width: 94px;text-align:right;">{{ $kredit  }}</td>
                <td style="width: 94px;text-align:right;">{{ $saldo }}</td>
