...
<table border="1" cellspacing="0">
    <tbody>
        ...
        @foreach($transaksi as $data)
            ...
            @if($no >= $rec1)
            <tr style="height:30px;">
                <td style="width: 108px;text-align:center;">{{ date('d/m/Y', strtotime($data->waktu)) }}</td>
                <td style="width: 70px;text-align:center;">{{ $data->sandi }}</td>
                <td style="width: 125px;text-align:right;">{{ $debit }}</td>
                <td style="width: 125px;text-align:right;">{{ $kredit  }}</td>
                <td style="width: 130px;text-align:right;">{{ $saldo }}</td>
